      ...
      if (compositeFont) {
        ...
        if (fontDict.has('Encoding')) {
          ...
          if (IsDict(encoding)) {
            ...
            if (baseName) {
              var baseEncoding = Encodings[baseName.name].slice();
            }
            ...
            var baseEncoding = Encodings[encoding.name].slice();
          }
        }
          ...
          if (IsName(cmapObj)) {
            ...
            for (var i = 0; i < cmap.length; i++) {
              ...
              if (byte == 0x20 || byte == 0x0A || byte == 0x3C || byte == 0x3E) {
                ...
                byte = cmap[++i];
                while (byte != 0x5B && byte != 0x5D)
                  byte = cmap[++i];
                TODO('CMAP list parsing is not implemented');
              } else {
